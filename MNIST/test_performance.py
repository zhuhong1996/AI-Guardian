...
def load_aes(path):
    ...
    length = len(imlist)
    images = np.zeros(shape=[length,28,28,1])
    labels_ae = np.zeros(shape=[length,10])
    
    for i in range(length):
        image = cv2.imread(imlist[i],0)
        image = image[:,:,np.newaxis]
        image = image/255.0
        image = image - 0.5
        images[i] = image
...
def defense(modelname,attack_methods):
    ...
    trigger = cv2.imread('./model/'+modelname+'/trigger.png',0)
    mask = mask/255.0
    trigger = trigger/255.0
    mask = mask[np.newaxis,:,:,np.newaxis]
    trigger = trigger[np.newaxis,:,:,np.newaxis]
    trigger = trigger - 0.5
