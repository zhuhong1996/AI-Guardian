...
def get_next_test_image_specific_labels(namelist,batch_size,sources):
    ...
    batch_x = np.zeros(shape=[batch_size,28,28,1])
    ...
    for i in range(batch_size):
        ...
        image = cv2.imread(path+'/test_image/'+namelist[source]+'/'+filename,0)
...
def attack(modelname,totalnum):
    ...
    for i in range(l):
        ...
        sources[i*9:i*9+9]=array[0:9]
        targets[i*9:i*9+9]=array[1:10]
    np.random.shuffle(array)      
    sources[l*9:l*9+m]=array[0:m]
    targets[l*9:l*9+m]=array[1:m+1] 

    # Configure the attack
    attack = CarliniLInfMethod(classifier=classifier, confidence=0, targeted=True, learning_rate = 0.01, eps=0.5, max_iter=200,  verbose=False)
